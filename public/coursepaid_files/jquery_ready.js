hs.graphicsDir = '//www.tienganh123.com/highslide/graphics/';
...
$(document).ready(function(){
    ...
    $("a.replay").live("click",function(){
        ...
        if($(this).text() == "Ẩn")
        {
            ...
            var textOld = "<span class=\"blue1\">Trả lời : </span><b>"+ numrlNew +"</b>";
    ...
    //Back Top
    $("#tops").hover(function(){
        $(this).css("background","url(//data.tienganh123.com/images/v2/home/img_homepage.png) no-repeat scroll -220px -455px transparent")
        },function(){
            $(this).css("background","url(//data.tienganh123.com/images/v2/home/img_homepage.png) no-repeat scroll -170px -455px transparent")
    })
        ...
        $.post(base_urlroot+"ajax/hoidapnhanh",params,function(rs){            
            if(IsJsonString(rs))
            {
                ...
                else if(rs.type == "success")
                {
                    ...
                    $.get( "/ajax/hoidapnhanh/load_faq", {id: rs.id} )
                    .done(function( _data ) {
        ...
        $.post(base_urlroot+"ajax/hoidapnhanh",params,function(rs){
            if(IsJsonString(rs))
            {
                ...
                else if(rs.type == "success")
                {
                    ...
                    textOld = rlNew;
                    chat_home_socket.emit('send_replay', {rs: rs, numrlNew: numrlNew, idComment: idComment});
                }
            }
            else
            {
        ...
        if(confirm("Bạn chắc chắn xóa trả lời này ?"))
        {
            ...
            $.post(base_urlroot + "ajax/hoidapnhanh",params,function(rs){
                ...
                {   
                    ...
                    $("input#send_form_comment[class='"+ idHoiDap +"']").attr("alt",num);
                    chat_home_socket.emit('del_answer', {rs: rs, numrlNew: num, idReplay: idReplay, idHoiDap: idHoiDap}); 
                }
                else
    ...
    $(".setSpam").live("click",function(){
        if(confirm("Áp dụng Spam cho tài khoản này ?")){
            ...
            $(this).parent().parent().find("span.font_weight").css({"border-color": "#FF0000", 
             "border-width":"2px", 
             "border-style":"solid"});
            $.post(base_urlroot + "ajax/hoidapnhanh",params,function(rs){
    ...
    $(".actiHD").live("click",function(){
        if(confirm("Duyệt câu hỏi này được phép hiển thị ra bên ngoài ?")){
            ...
            $.post(base_urlroot + "ajax/hoidapnhanh",params,function(){
                $('.actiHD[rel='+ IdQs +']').html('&nbsp;<img src="//data.tienganh123.com/images/v2/home/accept-icon.png">') 
            })
        }
        else return false;
    })
    ...
    $("#repassword").blur(function(){
        ...
        if(password.length < 6){
            ...
            $("#er_repassword").show()         
            $("#er_repassword").text("Bạn hãy nhập lại - mật khẩu phải có ít nhất 6 ký tự")
...
function show_button_download(){  
    if($('#download_1').length==1){ 
        ...
        while($('#download_'+i).length==1){ 
            ...
            non_vip+='<a href="" onclick="popup(\'//data.tienganh123.com/file/dungchung/download_vip.htm\')" onmouseout=hideticker(this) onMouseOver="showmessage_danhngon(\'Tải video, audio, hay lời dịch về máy - Chỉ dành cho thành viên VIP\',this)"><div class="vd_dl_icon">'+arr_link[0]+'<div class="vd_ico_dl '+icon_downd+'"></div></div></a>';
...
function login_openid(identity){
    ...
    if(identity != 'yahoo')
        ...
        $html = '<form style="display:none;" action="'+ base_urlroot +'/ajax/oauth/openid" method="post" id="openid_yahoo">';
            $html += '<input type="url" value="http://me.yahoo.com/" id="openid_url" name="openid_url">';
            $html += '<input type="submit" id="abcd" onclick="this.form.submit();" value="Authenticate">';
        $html += '</form>';
        $('body').append($html);
        $('#abcd').trigger('click'); 
    }        
        ...
        html2canvas($('.RsCus'),{
            ...
            $.ajax({ 
                ...
                data: {
                    ...
                    if(o.suc == 'ok'){
                        $.post( url, { base64data : imgString, filename: o.filename})
                        .done(function( data ) {
...
function _youtu_ads()
{
    $.post( "/ajax/page/hide_pop_youtube", {})
    .done(function(data) {
        console.log("done");
        return location.href = 'https://www.tienganh123.com/tin-tuc-tienganh123/22477-co-hoi-nhan-the-tieng-anh-123-mien-phi.html';
    });
}
...
function _tacbm_ads()
{
    ...
    var Time_expires=24*3600*1000;TVH_ADS_SetCookie('pop_tacbm','true',Time_expires);
    location.href = 'https://www.tienganh123.com/tieng-anh-giao-tiep-cho-nguoi-mat-goc';
    console.log('pop_tacbm');
}

function get_rand_video_youtube() {
    var _arr = ['ekJwhr3-l6M','wchPzrqYS4M','CdF2RYPdGRk','sjnryMaeWWI','2wcBDUphdxw','w3cYcG_QAgU','IHDw9Dg-fr4','i28scQ9jDOc','QeibQaYa4uQ','Kf-EiA6rwos','jEifGuo5ZPA','yQ1kSHHQFyg','-w9jwzsGhcg','jKNK-Xn2bos','LYqjF6I_vEg','y9dRsxuN22M','d20LCOsRL_E','jhA5D2Onbns','KDyRxCCswrI','SSYR8PtEWsE','GlNrv2_Acnk','UbkAnaE2fe4','vdkHFZzelyQ','p0z7kqfwgCU','OvRWJG1COm4','Zx7ors3kWXQ','bifr2385rco','Mtl3x6T3DgY','yqBwWBjgkeA','ThdMxKpwcVs','10qoDy0lD8c','eIa2j62_-R4','v8jmfXxFy04','Og3XK_yU1EY','vMvyOkIcFkQ','FtxEfaoyJzk','Uf9E9Y_us_Y','I4OnNQiOiLM','v38UFkJMko4','ywjhYE_5vgk','PEOYp-GvJx0','JNKxhcJZWM0','tntVqnLTMpU','lHgZIqyvTbo','6BeXsWh6xBQ','slQX2PG8DhI','KiBjqfvWCDQ','sIzrbTkDdKI','SEQmgfh0WnE','2zAzg7uoHnc','LiDDQVMR4JQ','n3rqpBtk65s','4oER_kUAnho','e_nc5ORKfRY','VD7iAsVMl0M','aIbCTJiJYxU','FG4RViiCHj4','PIPSC-UjHPs','Ah8icRNs-h8','fUX0xZAJSOg','_iRVCg25Gqg','KZd9CdIGjis','ql1Cx9WsxOs','SBIcGYRpsQg','NiBWBDhe4AE','CRz3H6K4RfQ','v6_I8pQRhDo','tRj7QIIBMWA','r5zhpJwO0rk','gFZIXFtetoA','YU626cpEY3E','LPJOfW_Cvpg','W3AjnE_Alnw','akvpdNNZpro','IHDw9Dg-fr4','ULxKkcYQRjA','w2cEHQiUpa0','NF-YcoPHE6A','8BIdbhsbdZY','5CNPgxAiXUI','zqOlPuy3c9s','67WFHPkz9rI','VxBGwfKr3k0','SRsC9sJtfcM','fq1jhPhcAgc','NwkYDq7Gyk8','f7Pre-0mXLY','sGvf0-1ob7g','k6ii7GGtTK4','2hDu_9ReiW8','q6-a_TTUgSE','g4TkpmPkW18','nbFHpK4YReE','e7qPc8IoMJ8','G_JU8JbmZqA','2x08U4ZPz3g','YJeeuj_Ljag','QczLi9eXCxY','VcR_FCgwY-w','FVjGFH1HMdg','imKEO6K2_LM','hv6035KAUZo','W8VX6fpj9eg','RQkiv7jbWDM','EpG90k4N6pQ','UpdgGEej5FI','vuZi1AsTRoY','aCk9micTKiw','5UjC-XRmFqU','7Eqf9VQCoQQ','stQFq6w6TnI','JyPR6v4qmd0','P4-k1_7705g','7NgH0rsgh2k','271_wlvI-KI','cRgWPgtNGwg','QC_6RZ_1zoI','Jkj3ZI46pxk','d7fz4hCJ4-U','8nwm6D_23hM','tVFd7vfSNZo','1JniBEstIao'];
    return _arr[Math.floor(Math.random() * _arr.length)];
}  

var lt_arr = [];
lt_arr[0] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-lop-3', 'text': 'Giải bài tập SGK <strong>Toán 3</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-tieng-viet-lop-3', 'text': 'Soạn <strong>Tiếng Việt lớp 3</strong>'}, 2: {'link': 'https://www.chuabaitap.com/ ', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[1] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-lop-4', 'text': 'Giải bài tập SGK <strong>Toán 4</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-tieng-viet-lop-4', 'text': 'Soạn <strong>Tiếng Việt lớp 4</strong>'}, 2: {'link': 'https://www.chuabaitap.com/ ', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[2] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-lop-5', 'text': 'Giải bài tập SGK <strong>Toán 5</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-tieng-viet-lop-5', 'text': 'Soạn <strong>Tiếng Việt lớp 5</strong>'}, 2: {'link': 'https://www.chuabaitap.com/ ', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[3] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-6', 'text': 'Giải bài tập SGK <strong>Toán 6</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-van-lop-6-chi-tiet', 'text': 'Soạn <strong>Ngữ Văn 6</strong>'}, 2: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-vat-ly-6', 'text': 'Giải bài tập SGK <strong>Vật Lý 6</strong>'}, 3: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-lich-su-6', 'text': 'Giải bài tập SGK <strong>Lịch Sử 6</strong>'}, 4: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-sinh-hoc-6', 'text': 'Giải bài tập SGK <strong>Sinh Học 6</strong>'}, 5: {'link': 'https://www.chuabaitap.com/', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[4] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-7', 'text': 'Giải bài tập SGK <strong>Toán 7</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-van-lop-7-chi-tiet', 'text': 'Soạn <strong>Ngữ Văn 7</strong>'}, 2: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-vat-ly-7', 'text': 'Giải bài tập SGK <strong>Vật Lý 7</strong>'}, 3: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-lich-su-7', 'text': 'Giải bài tập SGK <strong>Lịch Sử 7</strong>'}, 4: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-sinh-hoc-7', 'text': 'Giải bài tập SGK <strong>Sinh Học 7</strong>'}, 5: {'link': 'https://www.chuabaitap.com/', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[5] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-8', 'text': 'Giải bài tập SGK <strong>Toán 8</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-van-lop-8-chi-tiet', 'text': 'Soạn <strong>Ngữ Văn 8</strong>'}, 2: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-vat-ly-8', 'text': 'Giải bài tập SGK <strong>Vật Lý 8</strong>'}, 3: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-lich-su-8', 'text': 'Giải bài tập SGK <strong>Lịch Sử 8</strong>'}, 4: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-sinh-hoc-8', 'text': 'Giải bài tập SGK <strong>Sinh Học 8</strong>'}, 5: {'link': 'https://www.chuabaitap.com/', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[6] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-9', 'text': 'Giải bài tập SGK <strong>Toán 9</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-van-lop-9-chi-tiet', 'text': 'Soạn <strong>Ngữ Văn 9</strong>'}, 2: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-vat-ly-9', 'text': 'Giải bài tập SGK <strong>Vật Lý 9</strong>'}, 3: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-lich-su-9', 'text': 'Giải bài tập SGK <strong>Lịch Sử 9</strong>'}, 4: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-sinh-hoc-9', 'text': 'Giải bài tập SGK <strong>Sinh Học 9</strong>'}, 5: {'link': 'https://www.chuabaitap.com/', 'text': 'Xem các lớp khác và môn khác'}};
lt_arr[7] = {0 : {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-toan-10', 'text': 'Giải bài tập SGK <strong>Toán 10</strong>'}, 1: {'link': 'https://www.chuabaitap.com/soan-van-lop-10-chi-tiet', 'text': 'Soạn <strong>Ngữ Văn 10</strong>'}, 2: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-vat-ly-10', 'text': 'Giải bài tập SGK <strong>Vật Lý 10</strong>'}, 3: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-lich-su-10', 'text': 'Giải bài tập SGK <strong>Lịch Sử 10</strong>'}, 4: {'link': 'https://www.chuabaitap.com/giai-bai-tap-sgk-sinh-hoc-10', 'text': 'Giải bài tập SGK <strong>Sinh Học 10</strong>'}, 5: {'link': 'https://www.chuabaitap.com/', 'text': 'Xem các lớp khác và môn khác'}};
var item = lt_arr[Math.floor(Math.random()*lt_arr.length)];
var _str = '';
$.each(item, function( index, value ) {
    var _text = value.text;
    _text = _text.replace(/(<([^>]+)>)/ig,"");
  _str += '<li><span class="bull_icon">•</span><a title="'+_text+'" target="_blank" href="'+value.link+'">'+value.text+'</a></li>';
});
...
function ta123pop(){
    ...
    if(cookie_popup_ads==''){
        if(tvhayorg==0){
            ...
            var time = d.getTime();
            var Time_expires=time + 24*3600*1000;TVH_ADS_SetCookie('abckid_popup_ads','true',Time_expires);
            ...
            var pop_ads_open2=window.open('https://www.abckid.vn','ta123_ads_pop',params+', top=0,left=0');
        }
    }
}
