    ...
    if(confirm("Bạn có chắc chắn muốn cập nhật thông tin?")){
        $.post(base_urlroot + "member/" + member_id + "/info",params,function(rs){
            ...
            if(rs.result){
                ...
                $('#form_show_result').find('image').attr('src','/images/pne_friend.png');

            }else{
                $('#result_message').html(rs.message);
                $('#form_show_result').find('image').attr('src','/images/home/rs_note_false.png');
            }
...
function status_new_comment()
{
    if (xmlhttp.readyState==4)
    {
        ...
        if(str[1]!='')
            $('#show_error_msg').css('color','red').html(str[1]);
        else{
...
function status_blog_comment()
{
    if (xmlhttp.readyState==4)
    {
        if(xmlhttp.responseText == 'novipkid')
             document.getElementById("comment_ico_title").innerHTML='<span style="font-size:13px;color:red">Chỉ thành viên VIP  mới có thể ghi lên bảng!</span>';
        else
        {
